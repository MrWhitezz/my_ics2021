...
static def_DHelper(CSR) {
  ...
  decode_op_csr(s, id_src2, s->isa.instr.CSR.csr, false); // flag not used; id_src2 used as csr
}

def_THelper(R_Mul_Div) {
  def_INSTR_TAB("??????? ????? ????? 000 ????? ????? ??", mul);
  def_INSTR_TAB("??????? ????? ????? 001 ????? ????? ??", mulu);
...
def_THelper(computeR) {
  def_INSTR_TAB("0000001 ????? ????? ??? ????? ????? ??", R_Mul_Div);
...
def_THelper(CtrlStatus) {
  def_INSTR_TAB("0000000 00000 ????? 000 ????? ????? ??", ecall);
  def_INSTR_TAB("0011000 00010 ????? 000 ????? ????? ??", mret);
...
def_THelper(main) {
  ...
  def_INSTR_IDTAB("??????? ????? ????? ??? ????? 11100 11", CSR   , CtrlStatus);
